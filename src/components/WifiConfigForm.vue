  ...
  <form @submit.prevent="generateQrCodeData">
    ...
    <div v-if="wifiConfig.type !== 'nopass'" class="mb-6">
      ...
      <div class="flex gap-2">
        <input id="password" :type="passwordVisible ? 'text' : 'password'" v-model="wifiConfig.password" required class="block w-full rounded-md shadow-sm py-3 px-6 sm:text-sm">

        <button @click="passwordVisible = !passwordVisible" type="button" class="bg-white rounded-md shadow-sm p-3 sm:text-sm">
