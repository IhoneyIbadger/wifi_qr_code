        ...
        .from(`
          <div class="flex justify-center text-white">
            <div class="flex flex-col bg-indigo-500 shadow-sm rounded-lg p-6">
              ...
              <span class="block text-xl text-center">Wifi Name / SSID: ${this.$props.qrCodeData.name}</span>
              <span class="block text-center mb-4">Scan with your phone to connect!</span>
              <span class="block text-sm text-center">Generated on: <a href="https://wifi2qr.com">wifi2qr.com</a></span>
            </div>
          </div>
        `, 'string')
        .toPdf()
        ...
        .from(`
          <div class="flex justify-center text-white">
            <div class="flex flex-col bg-indigo-500 shadow-sm rounded-lg p-6">
              ...
              <span class="block w-80 text-xl text-center">Wifi Name: ${this.$props.qrCodeData.name}</span>
...
<template>
  <div class="flex justify-center space-x-4">
    <div class="flex flex-col bg-indigo-500 shadow-sm rounded-lg p-4">
      ...
      <span v-if="qrCodeData.name" class="w-64 text-xl text-white text-center">Wifi Name: {{qrCodeData.name}}</span>
    </div>

    <div v-if="previewUrl" class="flex flex-col justify-center grow-1">
      <button @click="downloadPdf" type="button" class="flex items-center py-4 px-8 border-b border-b-gray-500">
      ...
      <button @click="downloadImage" type="button" class="flex items-center py-4 px-8 border-b border-b-gray-500">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="mr-2" viewBox="0 0 16 16">
          <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"/>
          <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"/>
        </svg>
      ...
      <button @click="print" type="button" class="flex items-center py-4 px-8">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="mr-2" viewBox="0 0 16 16">
